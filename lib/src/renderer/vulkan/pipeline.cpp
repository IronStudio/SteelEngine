		...
		m_pipelineLayout {VK_NULL_HANDLE}
	{
		...
		if (m_infos.type == se::renderer::PipelineType::eRasterization) {
			...
			if (vkCreatePipelineLayout(device, &pipelineLayoutCreateInfos, nullptr, &m_pipelineLayout) != VK_SUCCESS)
				throw se::exceptions::RuntimeError("Can't create graphics pipeline layout");
		...
		// compute pipeline
			...
			SE_ASSERT(m_infos.shaders.size() == 1, "Compute pipeline must have exactly one shader stage");
			...
			if (vkCreatePipelineLayout(device, &pipelineLayoutCreateInfos, nullptr, &m_pipelineLayout) != VK_SUCCESS)
				throw se::exceptions::RuntimeError("Can't create compute pipeline layout");

			VkPipelineShaderStageCreateInfo shaderStageCreateInfos {};
			shaderStageCreateInfos = reinterpret_cast<const se::renderer::vulkan::Shader*> (m_infos.shaders[0])->getShaderStageCreateInfos();
			...
			if (vkCreateComputePipelines(device, VK_NULL_HANDLE, 1, &pipelineCreateInfos, nullptr, &m_pipeline) != VK_SUCCESS)
		...
		// raytracing pipeline
			...
			shaderGroupCreateInfos.sType = VK_STRUCTURE_TYPE_RAY_TRACING_SHADER_GROUP_CREATE_INFO_KHR;
			shaderGroupCreateInfos.anyHitShader = ;
			shaderGroupCreateInfos.closestHitShader = ;
			shaderGroupCreateInfos.generalShader = ;
			shaderGroupCreateInfos.intersectionShader = ;
			shaderGroupCreateInfos.type = VK_RAY_TRACING_SHADER_GROUP_TYPE_TRIANGLES_HIT_GROUP_KHR;
			shaderGroupCreateInfos.pShaderGroupCaptureReplayHandle = ;

			VkRayTracingPipelineInterfaceCreateInfoKHR pipelineInterfaceCreateInfos {};
			pipelineInterfaceCreateInfos.sType = VK_STRUCTURE_TYPE_RAY_TRACING_PIPELINE_INTERFACE_CREATE_INFO_KHR;
			pipelineInterfaceCreateInfos.maxPipelineRayPayloadSize = 0;
			pipelineInterfaceCreateInfos.maxPipelineRayHitAttributeSize = 0;

			VkRayTracingPipelineCreateInfoKHR pipelineCreateInfos {};
			pipelineCreateInfos.sType = VK_STRUCTURE_TYPE_RAY_TRACING_PIPELINE_CREATE_INFO_KHR;	*/
		}
	}
